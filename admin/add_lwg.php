...
$carikode = mysqli_query($connect, "SELECT id_lowongan FROM lowongan") or die(mysqli_error($connect));
// Mengambil data terakhir dari tabel lowongan
$datakode = mysqli_fetch_array($carikode);
$jumlah_data = mysqli_num_rows($carikode);

// Jika ada data di tabel lowongan
if ($datakode) {
    // Ambil 4 karakter terakhir dari id_lowongan (misal: LWG0001 -> 0001)
    $nilaikode = substr($datakode['id_lowongan'], 3);
    // Ubah $nilaikode menjadi integer dan tambahkan 1
    $kode = (int) $nilaikode + 1;
    // Buat kode otomatis dengan format LWGXXXX
    $kode_otomatis = "LWG" . str_pad($kode, 4, "0", STR_PAD_LEFT);
} else {
    // Jika tidak ada data, mulai dari LWG0001
